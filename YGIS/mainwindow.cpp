...
{
    QString filePath = QFileDialog::getOpenFileName(this, QString(), QString(), "Image Files (*.bmp *.jpg *.png);;Shapefile (*.shp)");
    if (filePath.endsWith(".shp"))
    {
        GDALAllRegister();
        GDALDataset* poDataset = (GDALDataset*)GDALOpenEx(filePath.toStdString().c_str(), GDAL_OF_VECTOR, NULL, NULL, NULL);
        ...
        while ((poFeature = poLayer->GetNextFeature()) != nullptr)
        {
            ...
            if (poGeometry != nullptr && wkbFlatten(poGeometry->getGeometryType()) == wkbPoint)
            {
                ...
                QPoint qPoint(poPoint->getX(), poPoint->getY());
                points.append(qPoint);
                primitive = new Primitive(pen, Primitive::Point, { qPoint });
                primitives.append(primitive);
            }
